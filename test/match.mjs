...
async function main() {
  ...
  host.s.emit('game:setTarget', { target: 10 })
  await sleep(300)
  console.log('target after set:', host.state.room.matchTarget)
  if (host.state.room.matchTarget !== 10) throw new Error('setTarget did not apply')

  host.s.emit('game:setTarget', { target: 0 }) // out of range
  await sleep(300)
